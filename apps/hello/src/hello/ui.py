"""hello's two front-ends. Thin: fetch via :func:`hello_info`, render a template."""
...
from hello.api import hello_info
...
def make_ui_router(desktop: Jinja2Templates, mobile: Jinja2Templates) -> APIRouter:
    ...
    def desktop_index(request: Request) -> HTMLResponse:
        return desktop.TemplateResponse(request, "index.html", {"info": hello_info()})

    @router.get("/m/", response_class=HTMLResponse)
    def mobile_index(request: Request) -> HTMLResponse:
        return mobile.TemplateResponse(request, "index.html", {"info": hello_info()})
